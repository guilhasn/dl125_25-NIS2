...
> «A entidade implementa controlo de acessos baseado em privilégios mínimos, autenticação multi-factor para acessos administrativos e remotos, e gestão documentada do ciclo de vida das credenciais.» — Anexo IV, medidas **O.PAP** + **O.PP** + **T.AM** (síntese aplicável a Grupos A e B)

## MFA: obrigatório para acessos administrativos

A **autenticação multi-factor (MFA)** é o controlo individual de maior impacto. Estima-se que **bloqueie 99% dos ataques baseados em credenciais** — não pelo factor adicional ser mágico, mas porque o **custo do atacante** sobe para níveis em que ataques massivos deixam de compensar.

**Onde tem de estar activo, sem excepção**:
...
**Onde é fortemente recomendado mas pode haver excepções fundamentadas**:
...
O conselho tradicional («palavras-passe complexas com símbolos, rotação trimestral, mínimo 8 caracteres») foi **revisto pelo NIST** em 2017 e a tendência consolidou-se na Europa. A regra actual é mais simples e mais segura:

- **Comprimento mínimo**: **12 caracteres** (idealmente **15+**). Caracteres especiais opcionais.
- **Sem rotação obrigatória** (excepto após suspeita de compromisso). Rotações forçadas levam os utilizadores a padrões previsíveis (`Verao2025!`, `Verao2026!`).
...
A maior parte das contas em câmaras portuguesas tem **mais permissões do que deveria**. Trabalhador da contabilidade que tem acesso ao módulo de recursos humanos «porque um dia precisou». Antigo chefe de divisão que ainda é administrador do domínio «porque ninguém revogou». Estagiária que ainda tem conta activa três anos depois de sair.

**Regra**: cada utilizador tem **exactamente as permissões necessárias** para a função actual. Nem mais, nem menos.
...
Antes de avançar para D.4, momento curto de revisão. Em quantas das suas **contas privilegiadas** (M365, VPN, administração de servidores, Active Directory, aplicações financeiras) tem **MFA activo neste momento**?
