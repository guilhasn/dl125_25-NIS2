---
title: "Aviso n.º 5146/2026/2"
layout: default
parent: "Legislação"
nav_order: 2
---

# Aviso n.º 5146/2026/2 — Regulamento do CNCS
...
Aprovação do regulamento de execução do RJC.
...
### `art. 5.º` a `art. 7.º` Autenticação {#art-5-7}

Acesso por **Cartão de Cidadão** ou **Chave Móvel Digital**.

### `art. 8.º` Auto-identificação {#art-8}

Procedimento de qualificação na plataforma. Prazo de 60 dias após a disponibilização (concretiza o art. 8.º RJC).

### `art. 9.º` Qualificação pelo CNCS {#art-9}

Após auto-identificação, o CNCS confirma ou ajusta a qualificação por decisão notificada na plataforma.

### `art. 12.º` API {#art-12}

Disponibilidade opcional de API para automação por entidades com capacidade técnica.
...
## Aviso — Capítulo III: Notificação de incidentes (arts. 20.º a 22.º) {#cap-3}

Detalha a tramitação prática das notificações previstas nos arts. 40.º a 44.º RJC.

### `art. 20.º` Procedimento {#art-20}

Detalha o fluxo: login → área reservada → módulo «Notificação de incidentes» → tipo de notificação → formulário → anexos → submissão → ID único → acknowledge.
...
> «O ID único atribuído à notificação inicial é o referencial para todas as notificações subsequentes relacionadas com o mesmo incidente.»

**N.º 3** habilita alertas automatizados quanto aos prazos seguintes.

Aplicação: [C.3 Conteúdo de cada notificação]({% link 02-notificacao-incidentes/c3-conteudo-notificacao.md %}).

## Aviso — Capítulo IV: Instrumentos estruturantes (arts. 23.º a 33.º) {#cap-4}
...
Detalha os cenários da matriz para essenciais/importantes.

### `art. 31.º` Periodicidade da análise de risco residual {#art-31-aviso}

> «As entidades essenciais e importantes realizam a análise do risco residual com a seguinte periodicidade: a) Durante o planeamento e introdução de alteração (...); b) Após a ocorrência de um incidente com impacto significativo (...); c) Após a notificação, por parte do CNCS, de um risco, ameaça ou vulnerabilidade emergente.»
...
Remete para o Anexo IV — as medidas concretas estão no Anexo, não no articulado.

## Aviso — Capítulo V: Disposições finais (arts. 34.º a 35.º) {#cap-5}

### `art. 34.º` Norma transitória {#art-34}

Disposições para a adaptação.

### `art. 35.º` Entrada em vigor {#art-35}

5 dias após publicação da versão final no DR.

## Aviso — Anexos {#aviso-anexos}
...
Medidas detalhadas com níveis Básico, Substancial e Elevado. Aplica-se a SMAS qualificado como essencial — ver [Roteiro essencial — Anexo III vs Anexo IV]({% link roteiro-essencial/anexo-iii-vs-anexo-iv.md %}).

### Anexo IV — Medidas para entidades públicas relevantes (Grupo A/B) {#anexo-iv-aviso}

**Anexo central para autarquias.** As 27 medidas O/T/H, com aplicabilidade por Grupo A/B. Reproduzido e comentado em [Anexo IV — medidas O/T/H]({% link recursos/anexo-iv-aviso-5146.md %}).
