# Formação NIS2 — DL 125/2025

Repositório do manual digital público da formação sobre o **Decreto-Lei 125/2025** (regime jurídico da cibersegurança) e o [*Aviso 5146/2026/2*{:.legal}]({% link recursos/legislacao-aviso-5146.md %}) (regulamentação do CNCS) aplicado a **autarquias locais portuguesas**.

- **Manual digital**: https://guilhasn.github.io/dl125_25-NIS2/
- **Formador**: Nuno Salvador
- **Data**: 15 de Junho de 2026
- **Duração**: 6h (09:30–12:30 + 14:00–17:00)
- **Público**: dirigentes e técnicos superiores de autarquias locais

## Estrutura do dia

O manual digital espelha a agenda. Cada bloco tem a estrutura **O QUE A LEI DIZ → O QUE FAZER → TEMPLATE**.

| Bloco | Tema |
|---|---|
| A | Enquadramento legal |
| B | Art. 27.º — gestão de risco e ativos |
| C | Notificação de incidentes |
| D | Outras medidas (fornecedores, continuidade, acessos, formação) |
| E | Supervisão, sanções, roadmap |

Para autarquias que operam **serviços de água, resíduos ou outros sectores do Anexo I** do DL, existe um **Roteiro essencial** paralelo.

## Templates protegidos

Os templates descarregáveis estão protegidos com password. Os formandos receberam-na por email antes da formação.

## Licença

Conteúdo: **CC BY 4.0** — partilha livre com atribuição. Os templates respeitam a licença original dos seus autores.
